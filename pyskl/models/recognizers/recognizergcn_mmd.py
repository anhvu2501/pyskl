...
class RecognizerMMDLoss(BaseRecognizer):
    """GCN-based recognizer for skeleton-based action recognition using new MMD loss """
    ...
    def forward_train(self, keypoint, label, **kwargs):
        ...
        cls_score, z = self.cls_head(x)
        gt_label = label.squeeze(-1)
        loss = self.cls_head.loss(cls_score, z, gt_label) 
        losses.update(loss)
